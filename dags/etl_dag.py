...
with DAG(
    ...
    dim_payment_methods = TrinoOperator(
        ...
        sql="""
            INSERT INTO iceberg.analytics.dim_payment_methods (
                method_name,
                code
            )
            SELECT
                pm.transactionType AS method_name,
            ...
            WHERE NOT EXISTS (
                SELECT 1
                FROM iceberg.analytics.dim_payment_methods existing
                WHERE existing.method_name = pm.transactionType
            );
        """,
    ...
    dim_currency = TrinoOperator(
        ...
        sql="""
            ...
            SELECT
                ...
                xxhash64(new_currencies.currency) AS code,
...
stg_payments = TrinoOperator(
    ...
    sql="""
        ...
        LEFT JOIN hive.flight.bookings b
           ...
           AND DATE(p.paid_at) = DATE(b.booking_date)
        LEFT JOIN iceberg.analytics.payment_methods_summary pm
